            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex items-center gap-2">
                  ...
                  {/* REVIEW BUTTON */}
                  {isCompleted && !s.reviewGiven && (
                    <Link
                      href={`/dashboard/review/${s.id}`}
                      className="px-4 py-2 rounded-full text-sm font-medium bg-[#4a5e27] text-white hover:opacity-90"
                    >
                      Leave Review
                    </Link>
                  )}
...
function HostControls({ session, onDone }: any) {
  ...
  async function end() {
    ...
    setLoading(false);
    onDone();
    window.location.href = `/dashboard/review/${session.id}`;
  }

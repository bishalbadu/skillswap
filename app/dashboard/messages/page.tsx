...
export default function MessagesPage() {
  ...
  async function updateStatus(requestId: string, action: "accept" | "reject") {
    ...
    <div className="p-10 space-y-6">
      ...
      {suspended && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
          🚫 Your account is currently suspended. You can view messages, but
      ...
      <div className="flex gap-3">
        <button
          ...
          className={`px-4 py-2 rounded ${
            tab === "received" ? "bg-[#4a5e27] text-white" : "border"
          }`}
        >
          Received Requests
        </button>
        ...
        <button
          onClick={() => setTab("sent")}
          className={`px-4 py-2 rounded ${
            tab === "sent" ? "bg-[#4a5e27] text-white" : "border"
          }`}
        >
          Sent Requests
        </button>
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col gap-2">
               ...
               {r.session && r.computedStatus !== "REJECTED" && (
  <Link
    href={`/dashboard/chat?userId=${r.requester.id}`}
    className="px-4 py-2 rounded-lg text-sm font-medium bg-[#7e9c6c] text-white hover:bg-[#6c875e] transition text-center"
  >
    💬 Chat
  </Link>
)}
            ...
            {r.message && (
              <div className="text-sm italic text-gray-700">“{r.message}”</div>
            )}
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              {r.computedStatus === "ACCEPTED" && (
                <span className="text-green-600 font-semibold">Accepted</span>
              )}

              {r.computedStatus === "REJECTED" && (
                <span className="text-red-500 font-semibold">Rejected</span>
              )}

              {r.computedStatus === "EXPIRED" && (
  <span className="text-yellow-600 font-semibold">Expired</span>
)}
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col gap-2">
                ...
                {r.session && r.computedStatus !== "REJECTED" && (
                  <Link
                    href={`/dashboard/chat?userId=${r.receiver.id}`}
                    className="px-4 py-2 rounded-lg text-sm font-medium bg-[#7e9c6c] text-white hover:bg-[#6c875e] transition text-center"
                  >
                    💬 Chat
                  </Link>
